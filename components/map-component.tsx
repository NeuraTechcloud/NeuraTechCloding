...
export default function MapComponent({ vehicles, selectedVehicle, mapLayer, onVehicleSelect }: MapComponentProps) {
  ...
  useEffect(() => {
    ...
    import("leaflet").then((L) => {
      ...
      layersRef.current.google_roadmap = L.tileLayer("http://{s}.google.com/vt/lyrs=m&x={x}&y={y}&z={z}", {
        maxZoom: 20,
        subdomains: ["mt0", "mt1", "mt2", "mt3"],
        attribution: "&copy; Google Maps",
      })

      layersRef.current.google_satellite = L.tileLayer("http://{s}.google.com/vt/lyrs=s&x={x}&y={z}&z={z}", {
        maxZoom: 20,
        subdomains: ["mt0", "mt1", "mt2", "mt3"],
        attribution: "&copy; Google Maps",
      })
  ...
  useEffect(() => {
    ...
    import("leaflet").then((L) => {
      vehicles.forEach((vehicle) => {
        ...
        const popupContent = `
          <div class="font-sans bg-gray-900 text-white p-1 rounded-md">
            <h4 class="font-bold text-base mb-1 text-amber-400">${vehicle.name} - ${vehicle.plate}</h4>
            <p><strong>Status:</strong> ${vehicle.status}</p>
            <p><strong>Velocidade:</strong> ${vehicle.speed} km/h</p>
            <p class="max-w-xs"><strong>Endereço:</strong> ${vehicle.address}</p>
          </div>
        `
